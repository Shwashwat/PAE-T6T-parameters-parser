...
from BenchmarkingFuctions import timer
...
def merge_dicts(list_of_dicts : list[dict]) -> dict:
   ...
   for dictionary in list_of_dicts:
      for key in all_keys:
         merged_dict[key].append(dictionary.get(key , None))
...
def export_to_formatted_excel(dict_of_lists:dict , output_path:str):
   ...
   # rows = zip(*dict_of_lists.values())
   rows = zip_longest(*dict_of_lists.values(), fillvalue=None)

   # write rows while tracking widths
   for row in rows:
      ws.append(row)
...
# def get_n_save_settings(arguments):
def get_n_save_settings(folder:str, params:list[str] | None = None, extra_params=None, all_flag=False, name_save_file=None)-> tuple[dict , str]:
   ...
   if name_save_file:
      report_name = path.join(folder, name_save_file)
   else:
   ...
   dict_processed = master_param_dict if all_flag else {param:master_param_dict[param] for param in required_params}
...
if __name__ == "__main__":
    ...
    get_n_save_settings(parameters.folder, parameters.params, parameters.extra_params, parameters.all, parameters.name_save_file)
